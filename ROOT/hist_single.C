void hist_single(){
    ...
    TTree* tUSVD = new TTree();
    TTree* tMiddle1 = new TTree();
    TTree* tMiddle2 = new TTree();
    TTree* tMiddle3 = new TTree();
    TTree* tMiddle4 = new TTree();
    TTree* tMiddle5 = new TTree();
    TTree* tDet1 = new TTree();
    TTree* tDet2 = new TTree();
    TTree* tDet3 = new TTree();
    TTree* tDet4 = new TTree();
    TTree* tDet5 = new TTree();
    TTree* tDSVD1 = new TTree();
    TTree* tDSVD2 = new TTree();
    TTree* tFVD = new TTree();
    TTree* tNeutrinos = new TTree();
    ...
    if (!file || file->IsZombie()) {
        std::cout<<"Error with file, not opened!"<<std::endl;}
    else  {
        file->GetObject("USVD",tUSVD);
        file->GetObject("Middle_1",tMiddle1);
        file->GetObject("Middle_2",tMiddle2);
        file->GetObject("Middle_3",tMiddle3);
        file->GetObject("Middle_4",tMiddle4);
        file->GetObject("Middle_5",tMiddle5);
        file->GetObject("Det_1",tDet1);
        file->GetObject("Det_2",tDet2);
        file->GetObject("Det_3",tDet3);
        file->GetObject("Det_4",tDet4);
        file->GetObject("Det_5",tDet5);
        file->GetObject("DSVD_1",tDSVD1);
        file->GetObject("DSVD_2",tDSVD2);
        file->GetObject("FVD",tFVD);
        file->GetObject("neutrinos",tNeutrinos);
    }
    ...
    for(int i = 0; i<treeSelect.size();++i){
        ...
        if(treeName[i]=="Neutrinos"){
            ...
            treeSelect[i]->SetBranchAddress("nu_ix", &nu_ix);
            treeSelect[i]->SetBranchAddress("nu_iy", &nu_iy);
            ...
            hnuxy->GetXaxis()->SetTitle("x [m]");
            hnuxy->GetYaxis()->SetTitle("y [m]");
            hnuzy->GetXaxis()->SetTitle("z [m]");
            hnuzy->GetYaxis()->SetTitle("y [m]");
            hnuxz->GetXaxis()->SetTitle("x [m]");
            hnuxz->GetYaxis()->SetTitle("z [m]");
            
            hnui->GetXaxis()->SetTitle("x [mm]");
            ...
            outFile->WriteObject(hnuxy,"hnuxy");
            outFile->WriteObject(hnuzy,"hnuzy");
            ...
            TH1D* habsx = new TH1D(("habsx_"+treeName[i]).c_str(), ("habsx_"+treeName[i]).c_str(), 700, 0, 70000);
            TH1D* habsy = new TH1D(("habsy_"+treeName[i]).c_str(), ("habsy_"+treeName[i]).c_str(), 200, -1000, 1000);
            TH1D* habsz = new TH1D(("habsz_"+treeName[i]).c_str(), ("habsz_"+treeName[i]).c_str(), 800, -1000, 7000);
    ...
    for (int i=0;i<histMapPions.size()-1;i++) {
        std::cout<<histMapPions[treeName[i]]<<std::endl;
