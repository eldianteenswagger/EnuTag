void hists_last(){
    ...
    std::vector<TChain*> treeSelect = {tDSVD1,tDSVD2,tFVD,tNeutrinos};
    std::vector<std::string> treeName = {"DSVD1","DSVD2","FVD","Neutrinos"};
    std::vector<bool> treeVD = {false,true,true,true};
    ...
    std::string ExtraOut = "./OUT/Extra_LAST.root";
    ...
    for(int i = 0; i<treeSelect.size();++i){
        ...
        if(treeName[i]=="Neutrinos"){
            ...
            //out files
            std::string outFileName = "./OUT/"+treeName[i]+"_LAST.root";
